...
#
# This file rps-generate-buildinfo.sh is part of the Reflexive
...
##
##    This internal script generates some C file containing buildinfo
...
if [ -z $CARBURETTA ]; then
    ...
    if [ -z $CARBURETTA ]; then
	printf "%s missing carburetta parser generator, see carburetta.com/ and github.com/kingletbv/carburetta\n" $0 2>&1
	exit 1
    fi
    export CARBURETTA
fi
...
printf "// to restore %s from a  %%%%__buildinfo.c%%%% file edit there lines with @REFPERSYS_HOME.\n" $1
...
printf "/// this generated file used to be called __%s.c\n" timestamp
printf "/// but is now __buildinfo.c, a generated file by %s\n" $0
printf "/// this is generated by %s in %s git %s\n" $0 $PWD $(./rps-generate-gitid.sh -s)
## end of file rps-generate-buildinfo.sh
