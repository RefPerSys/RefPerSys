...
#include <libgccjit.h>
#else
#warning RPSCONF_WITHOUT_GCCJIT is provided
...
/* absolute path to carburetta.com tool */
const char *rpsconf_carburetta;
...
}				// end obsolete rpsconf_trash_push_
...
rpsconf_test_cxx_compiler (const char *cxx)
{
  ...
  strcpy (showvectobj, showvectsrc);
  {
    ...
    if (rpsconf_codegen_flags)
      snprintf (compilshowvect, sizeof (compilshowvect),
		"%s -c -Wall -Wextra %s %s -o %s",
		cxx, showvectsrc, rpsconf_codegen_flags, showvectobj);
    else
      snprintf (compilshowvect, sizeof (compilshowvect),
  ...
  rpsconf_should_remove_file (cxxexe, __LINE__);
  {
	...
	{
	  fprintf (stderr,
		   "%s no hello or file name from C++ test popen %s"
		   " [%s:%d]\n",
...
rpsconf_test_libgccjit_compilation (const char *cc)
{
  ...
  if (access (rpsconf_libgccjit_include_dir, F_OK))
    {
      fprintf (stderr, "%s:%d git %s {gccjit incdir} no %s (%m)\n",
	       __FILE__, __LINE__ - 1, rpsconf_gitid,
	       (rpsconf_libgccjit_include_dir
		? rpsconf_libgccjit_include_dir : "."));
      fflush (stderr);
    };
  snprintf (cmdbuf, sizeof (cmdbuf) - 1,
	    "%s -fPIC -g -O -shared -DRPSJIT_GITID='\"%s\"'"
	    " -I%s do-test-libgccjit.c -o %s -lgccjit",
	    cc, rpsconf_gitid,
...
rpsconf_emit_configure_refpersys_mk (void)
{
  ...
  if (rpsconf_libgccjit_include_dir)
    ...
    {
      fprintf (f, "\n### no libgccjit include directory [%s:%d] git %s\n",
	       __FILE__, __LINE__ - 1, rpsconf_gitid);
    };
  if (rpsconf_carburetta)
    {
      fprintf (f, "\n#### carburetta.com parser/scanner generator:\n");
      fprintf (f, "REFPERSYS_CARBURETTA=%s\n", rpsconf_carburetta);
    }
  else
    {
      fprintf (f, "\n### no carburetta generator found!\n");
    };
  fflush (f);
  ////
	  ...
	  while (!feof (fsrctmpconf))
	    {
	      ...
	      size_t nbrd = fread (cbuf, RPSCONF_BUFFER_SIZE, 1,
				   fsrctmpconf);
	      if (nbrd == 0)
		{
		  if (feof (fsrctmpconf))
...
rpsconf_usage (void)
{
  ...
  puts ("\t CARBURETTA=<parsertool>  # set the carburett.com generator, eg");
  puts ("\t                          # CARBURETTA=/usr/local/bin/carburetta");
  puts ("\t CARBURETTA_FLAGS=<flags> # flags passed to that carburetta\n");
...
{
  char *carb = getenv ("CARBURETTA");
  char *realcarb = NULL;
  if (carb && access (carb, X_OK))
    {
      fprintf (stderr,
	       "%s ignore non-executable CARBURETTA %s (%s)\n",
	       rpsconf_prog_name, carb, strerror (errno));
      carb = NULL;
    };
  while (!carb)
    {
      carb = rpsconf_readline ("carburetta.com parser generator: ");
      if (carb && access (carb, X_OK))
	continue;
    }
  realcarb = realpath (carb, NULL);
  if (!realcarb)
    {
      fprintf (stderr, "%s [git %s] failed to realpath '%s' - %s <%s:%d>\n",
	       rpsconf_prog_name, RPSCONF_GIT_ID, carb, strerror (errno),
	       __FILE__, __LINE__ - 2);
      exit (EXIT_FAILURE);
    }
  char *pcmdbuf = NULL;
  int clen = asprintf (&pcmdbuf, "%s --help", realcarb);
  if (clen < 0 || pcmdbuf == NULL)
    {
      fprintf (stderr, "%s [git %s] failed to asprintf (%s) <%s:%d>\n",
	       rpsconf_prog_name, RPSCONF_GIT_ID, strerror (errno),
	       __FILE__, __LINE__ - 2);
      exit (EXIT_FAILURE);
    }
  FILE *p = popen (pcmdbuf, "r");
  if (!p)
    {
      fprintf (stderr, "%s [git %s] failed to popen: %s (%s) <%s:%d>\n",
	       rpsconf_prog_name, RPSCONF_GIT_ID, pcmdbuf, strerror (errno),
	       __FILE__, __LINE__ - 2);
      exit (EXIT_FAILURE);
    };
  /**
   * the carburetta --help is expected to output to its stdout
   * something like:
   * Carburetta scanner and parser generator (C) 2020-2026 Kinglet B.V.
   * version 0.8.29 (development)
   * https://carburetta.com/
   *
   * carburetta <inputfile.cbrt> <flags>
   *
   **/
  const int commline = __LINE__ - 8;
  int lincnt = 0;
  bool foundw_scanner = false;
  bool foundw_parser = false;
  bool foundw_carburetta = false;
  bool foundw_generator = false;
  bool foundw_cbrt = false;
  do
    {
      char linbuf[256];
      memset (linbuf, 0, sizeof (linbuf));
      if (!fgets (linbuf, (int) sizeof (linbuf) - 4, p))
	break;
      if (lincnt++ > 20)
	break;
      if (strstr (linbuf, "parser"))
	foundw_parser = true;
      if (strstr (linbuf, "scanner"))
	foundw_scanner = true;
      if (strstr (linbuf, "generator"))
	foundw_generator = true;
      if (strstr (linbuf, "generator"))
	foundw_generator = true;
      if (strstr (linbuf, "cbrt"))
	foundw_cbrt = true;
      if (strcasestr (linbuf, "carburetta"))
	foundw_carburetta = true;
    }
  while (!feof (p));
  int fail = pclose (p);
...
main (int argc, char **argv)
{
  ...
  if (!optimflags)
	...
	("C++ optimization and code generation flags:", "-O1 -g -fPIC");
  ...
  //// parse our ~/.gitconfig
  {
    ...
    if (home)
      snprintf (gitconfpath, RPSCONF_PATH_MAXLEN, "%s/.gitconfig", home);
	...
	do
	  {
	    ...
	    int p = -1;
	    RPSCONF_BREAKPOINT ();
	    s = sscanf (lbuf, " [%30[a-zA-Z]]%n", name, &p);
	    RPSCONF_BREAKPOINT ();
	    if (s > 0 && p > 0)
	      {
