...
#define MYARGMAXLEN 64
  ...
  if (!plugarg || plugarg[0]==(char)0)
    RPS_FATALOUT("failure: plugin " << plugin->plugin_name
                 << " without argument; should be a non-empty string");
  ...
  _f.obreplprecedence
    = RPS_ROOT_OB(_7iVRsTR8u3D00Cy0hp); //repl_precedence∈symbol
  /// get the repl_operator superclass
  _f.obclassoper
    =  RPS_ROOT_OB(_6vcJz35mTam01zYLjL); //repl_unary_operator∈class
  ...
  std::lock_guard<std::recursive_mutex>
    gunewoper(*_f.obnewoper->objmtxptr());
  if (xtraname)
    {
