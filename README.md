...
[CLIPSrules](https://clipsrules.net/). Compatibility with CLIPSrules
syntax is not a goal (unless funded and contributed) in 2026.
...
**Don't expect [RefPerSys](http://refpersys.org/) to be a mature
  project.** It is not in summer 2026.
...
source directory** (hence have this `README.md` file and the
`refpersys.hh` header file). So if you did some command like `git
clone https://github.com/RefPerSys/RefPerSys` in your `$HOME/work/`
...
A *plugin* is some [shared
object](https://en.wikipedia.org/wiki/Library_(computing)#Shared_libraries)
(some `*.so` [ELF](https://wiki.osdev.org/ELF) file) loaded by
[dlopen(3)](https://man7.org/linux/man-pages/man3/dlopen.3.html).  The
C++ code of a plugin is hand-written, with the hope of needing less
and less of them (and have them replaced by automatically generated
module code). We don't bother
[dlclose(3)](https://man7.org/linux/man-pages/man3/dlclose.3.html)-ing
plugins. Several example plugins have their source code under
`plugins_dir/`; for example `plugins_dir/rpsplug_createclass.cc` is
creating a new persistent RefPerSys class (the comments inside give
details).

The `refpersys` executable accepts (optionally, in summer 2026) a
single *interactive plugin* (and example code might be in
`plugins_dir/rpsiplug_fox.cc` and uses the [fox
toolkit](https://fox-toolkit.org/).
...
You also should do a `make clean` after any `git pull` and often redo
a `make config`...
...
test09` is running `test_dir/009sepscript.bash` using
`test_dir/009sepscript.rps` as a script file passed with `--script`
program arguments. Run `make showtests` to list the testing targets
for GNU make.

The `make redump` command is redumping generated files and the
persistent state in situ. The `make altredump` is redumping under some
directory `/tmp/refpersys-*`. Run both very often to test the
