...
Rps_Dumper::scan_object(const Rps_ObjectRef obr)
{
  ...
  if (du_dumpobj == obr)
    return;
  if (du_dumpval && Rps_Value(obr) == du_dumpval)
    return;
...
Rps_Dumper::is_dumpable_objattr(const Rps_ObjectRef obr)
{
  if (!obr)
    return false;
  std::lock_guard<std::recursive_mutex> gu(du_mtx);
  RPS_ASSERT(du_callframe && du_callframe->is_good_call_frame());
  ...
  RPS_POSSIBLE_BREAKPOINT(); // reached in commit ced1132d56d9 (May 2026)
  _f.attrob = obr;
  ...
  if (!rpsmodfmt[0])
    {
      ...
      /// and the rps-generate-buildinfo.sh script.
...
Rps_Dumper::scan_every_source_file_for_constants(void)
{
  int nbscanedfiles = 0;
  int nbtotconsts = 0;
  for (const char*const*pcurfilename = rps_files;
       *pcurfilename; pcurfilename++)
    {
      const char*curpath = *pcurfilename;
      int lencurpath = strlen(curpath);
      /// Files inside generated/ are generated by some previous dump
...
Rps_Dumper::write_generated_roots_file(void)
{
  ...
  rps_emit_gplv3_copyright_notice(*pouts, /*path:*/rootpathstr,
                                  /*prefix:*/ "//:", /*suffix:*/"",
                                  /*owner:*/"", /*reason:*/"");
  *pouts << "/// generated by write_generated_roots_file from "
         << __FILE__ << ":" << rps_decimal_string(__LINE__) << std::endl;
  *pouts << "///! To add a new generated root, use rps_add_root_object"
         << std::endl
...
Rps_Dumper::write_generated_names_file(void)
{
  ...
  rps_emit_gplv3_copyright_notice(*pouts, rootpathstr,
                                  /*prefix:*/ "//:", /*suffix:*/"",
                                  /*owner:*/"", /*reason:*/"");
  *pouts << "/// generated by write_generated_names_file from "
         << __FILE__ << ":" << __LINE__ << std::endl;
  ///
  *pouts << "///! To add a new generated name, use rps_add_root_object"
         << std::endl
         << "///! or several plugins under plugins_dir/"
         << std::endl
         << "///! or Rps_PayloadSymbol:: register_name" << std::endl
         << "///! or Rps_PayloadSymbol:: register_strong_name" << std::endl
         << "///! or Rps_PayloadSymbol:: register_weak_name" << std::endl;
...
Rps_Dumper::write_generated_data_file(void)
{
  ...
  *pouts << "#ifndef RPS_DATA_INCLUDED\n" << "#define RPS_DATA_INCLUDED /*really!*/1" << std::endl;
  ...
  *pouts << "#define RPS_PATH_BYTE_SIZE " << rps_decimal_string(rps_path_byte_size) << std::endl;
  *pouts << "#define RPS_SC_VERSION " << rps_decimal_string(sysconf(_SC_VERSION)) << std::endl;
  *pouts << "#define RPS_XOPEN_VERSION " << rps_decimal_string(sysconf(_SC_XOPEN_VERSION)) << std::endl;
  *pouts << "#define RPS_POSIX_LINE_MAX " << rps_decimal_string(sysconf(_SC_LINE_MAX)) << std::endl;
  ...
  *pouts << std::endl << std::endl;
  ...
  // emit the libgccjit version constants
  {
    *pouts << "/// GCC libgccjit version constants at dump" << std::endl
           ...
           << std::endl
           << "#define RPS_LIBGCCJIT_PATCHLEVEL " << rps_decimal_string(gcc_jit_version_patchlevel())
           << std::endl;
  }
  ////
  {
...
#if 0 && TODO
  {
    ///Rps_TwoValues two =  _f.genob⋯.send(&_, rpskob_4Doq8xpQ0zi001mbBX, ⋯);
  }
#endif // 0 && TODO */
...
} // end Rps_Dumper::write_generated_parser_decl_file
...
#if 0 && TODO
  {
    ///Rps_TwoValues two =  _f.genob.send⋯(&_, rpskob_0sggcz6Pq110236mRm, ⋯);
  }
#endif // 0 && TODO */
