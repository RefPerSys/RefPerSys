  ...
  if (Fl::args(argc, argv, &argix, rps_fltk_arg_handler) == 0)
    {
      if (argix>0 && argix<argc)
        RPS_FATAL("rps_fltk_initialize failed to parse program arguments, argix=" << argix
                  << " bad program argument:" << argv[argix]);
      else
        RPS_FATAL("rps_fltk_initialize confused when parsing program arguments, argix#" << argix << ", argc=" << argc);
      std::string titlestr;
