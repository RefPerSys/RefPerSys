...
\author{Basile Starynkevitch\institute{France, email: basile@starynkevitch.net} 
...
\begin{abstract}
  ...
  free software running on Linux; it is a hobby but serious research project 
  ...
  work has been largely inspired by the works of J. Pitrat (see 
  \cite{Pitrat:2009:ArtifBeings} and \cite{Pitrat:1996:FGCS}).
\end{abstract}
...
.  The message is sent from the \emph{originator}
...
technique, would require the same number
...
method. Note that we do not need to form $A$
...
preconditioning (see), which can be implemented
