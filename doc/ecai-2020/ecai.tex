...
\title{REFPERSYS \\
A Reflective Persistent System}

\author{Basile Starynkevitch\institute{CEA, France, email: basile@starynkevitch.net} 
\and Abhishek Chakravarti \institute{India, email: chakravarti.avishek@gmail.com}
\and  Nimesh Neema\institute{India, email: nimeshneema@gmail.com} }
...
\begin{abstract}
  ...
  style for a paper for ECAI. The specifications for layout are
  described so that non-\LaTeX\ users can create their own style sheet
  to achieve the same layout. The source for the sample file is
  available for \LaTeX\ users. The PostScript and the PDF file is
  available for all. A Microsoft Word template is also available. The
  layout is identical to ECAI'16 papers. The publisher (IOS Press)
  will insert a footer for each page.
\end{abstract}
