...
\author {
  ...
  \and Nimesh \textsc{Neema}\thanks{
    ...
    206 Sundaram, 38/2, Biyabani, Indore 452002, India.}
}
...
of these concepts. \textbf{\RefPerSys~ is a long term\footnote{I don't
  ...
  targetted \emph{only} for \textsc{Linux x86-64} computers
  \cite{Linux-amd64-ABI:2018}, preferably compiled by a recent
  \href{http://gcc.gnu.org/}{\textsc{Gcc}} compiler (at least
  \href{https://gcc.gnu.org/gcc-8/}{\textsc{Gcc 8}} in november
  2019).}. A Linux system\footnote{My own
\texttt{ours.starynkevitch.net} computer, running
\textit{Debian/Unstable}, has 64 Gibytes of RAM, 24 cores (AMD 2970WX)
and terabytes of disk space, including a terabyte of SSD.}  with at
least 16 Gibytes of RAM, 4 \textit{x86-64} cores, and 220 Gibytes of
disk is required. The grand ambition of {\RefPerSys} is to become
later an infrastructure for some strong
\href{https://en.wikipedia.org/wiki/Artificial_general_intelligence}{AGI}
...
convince J.Pitrat that open source
...
\href{https://en.cppreference.com/w/c/atomic}{\texttt{\_Atomic}}
...
machinery. The insight is that {\RefPerSys} is running several worker
threads known to its garbage collector. Our \textbf{agenda}
\index{agenda} is the central mechanism of {\RefPerSys} feeding these
worker threads with some work to do, using \textbf{tasklets}
representing a small amount of work to be done.

\medskip

Each worker thread is indefinitely looping like this:
\begin{enumerate}
\item it runs rarely some householding processing, notably garbage
  collection work. This is where garbage collection gets synchronized.
\item it waits, if so needed, for the agenda to become non-empty
\item it choose a tasklet $\tau$ to run inside the agenda. That
  tasklet is removed from the agenda.
\item it runs that tasklet $\tau$ for a small amount of time (a few
  dozen of milliseconds, typically), called a \textbf{step}. Of course
  during that step the agenda can (and usually will) change, and
  perhaps the same tasklet $\tau$ would be added again into the
  agenda, with maybe several other tasklets. Or on the contrary,
  running $\tau$ could remove one or several other tasklets $\tau_1,
  \tau_2 \ldots$ from the agenda, and add other ones $\tau'_1, \tau'_2, \ldots$ there.
...
desktop). Of course the agenda mechanism requires synchronization thru
...
\ldots} and is also common in software enginneering\footnote{A typical
...
\texttt{\$hellochunk} and \texttt{\$msg} is given thru the
``macro-string''
...
persistent store. The expansion will be done thru some scripting
...
techniques. Libraries such as Ian Taylor's
\href{https://github.com/ianlancetaylor/libbacktrace}{\texttt{libbacktrace}}
...
(and internally for the GC, also quasi-value). The object model is
defining our classes, our single inheritance mechanism, our message
sending protocol.
...
called \emph{transient} values.
  ...
  \href{https://en.wikipedia.org/wiki/Abstract_data_type}{abstract
    data type}, either uses some ordering (e.g. in
  \href{https://en.wikipedia.org/wiki/Red-black_tree}{red-black
...
{\RefPerSys} takes advantage of some practical features\footnote{We don't really care if these features are not exactly standard C11 \cite{c11-standard:2011}.} of C on Linux x86-64:
...
\begin{itemize}
  ...
  \item Limited
    ...
    abilities. Assume we have two \texttt{struct}-ures definitions, so
    \texttt{struct s1} and \texttt{struct s2}. Assume that both
    \texttt{s1} and \texttt{s2} \emph{start} with the same
    \emph{common} fields \texttt{unsigned num;} then
    \texttt{void*ptr;} followed by \texttt{char str[24];}. Assume that
    a pointer \texttt{p} points to a valid memory zone, whose
    alignment (respectively size) are at least all of
    \texttt{alignof(struct s1)}, \texttt{sizeof(struct s1)},
    \texttt{alignof(struct s2)}, \texttt{sizeof(struct s2)}: so we
    have \texttt{alignof(typeof(*p)) >= alignof(struct s1) \&\&
      sizeof(*p) >= sizeof(struct s1)} and \texttt{alignof(typeof(*p))
      >= alignof(struct s2) \&\& sizeof(*p) >= sizeof(struct
      s2)}. Then: \texttt{((struct s1*)p)->num} and \texttt{((struct
      s2*)p)->num} both refer to the same memory location and number
    there; \texttt{((struct s1*)p)->ptr} and \texttt{((struct
      s2*)p)->ptr} both refer to the same memory location and pointer
    there; and of course \texttt{((struct s1*)p)->str} and
    \texttt{((struct s2*)p)->str} is the \emph{same} string. See also
    ...
    \href{https://gcc.gnu.org/onlinedocs/gcc/Unnamed-Fields.html}{unamed
      fields} ability.
    ...
    \item \href{https://en.wikipedia.org/wiki/Tail_call}{Tail call}
      ...
      \item Common
        ...
        \href{https://gcc.gnu.org/onlinedocs/gcc/Statement-Exprs.html}{statment
          exprs} (very useful),
        ...
        \href{https://gcc.gnu.org/onlinedocs/gcc/Other-Builtins.html}{other
          builtins}, may be used in {\RefPerSys} code.
\end{itemize}
...
\emph{C} code) of that function. The
...
or.} for multi-threading support. And each object carries
...
Thru their flexible attributes (each of them can be fetched, added,
...
his own free will. We believe, similarily, that \texttt{git} users
...
\begin{figure}[h]
  ...
  \begin{tabular}{ll}
    \hline
    $\Omega$ ``first name'' : & string \texttt{"Basile"} \\
    $\Omega$ ``last name'' : & string \texttt{"Starynkevitch"} \\
    $\Omega$ ``email'' : & string {\relsize{-1}{\texttt{"basile@starynkevitch.net"}}} \\
    $\Omega$ ``year of birth'' : & tagged integer \texttt{\#1959} \\
    $\Omega$ ``friends'' : & set {\relsize{-1}{\texttt{\{ \_6eO8kozzUh801dHVt7 \_7zpWFJ2npj001ZfVvq \}}}} \\
    \hline
  \end{tabular}
...
such data has to be the payload of that object.
...
directory\footnote{Of course, some other directory can be given thru
...
\textcolor{red}{@@TODO: should be written, e.g. explanation of the files under \texttt{persistore/} and of their JSON syntax}
