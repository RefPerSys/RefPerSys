...
\index{rps\_manifest.hjson@\texttt{rps\_manifest.hjson}} describing
...
{\relsize{-1}{\texttt{persistore/sp\_8J6vNYtP5E800eCr5q-rps.hjson}}}.
...
then each object of some given \textit{oid} is preceded by a comment
like \texttt{//+ob\textit{\textbf{oid}}}, for example an object of oid
\texttt{\_3fzIPzNlWFV01GGQSt} starts with a comment
       {\relsize{-1}{\texttt{//+ob\_3fzIPzNlWFV01GGQSt}}} line.
...
The figure \ref{fig:hjson-scalar-or-object-values} gives the \textsc{Hjson} syntax of
scalar values or object references in persisted state files.  The figure
\ref{fig:hjson-composite-values} \textcolor{red}{is @INCOMPLETE@ and} gives
the \textsc{Hjson} syntax of composite values in persisted state files.
...
\begin{figure}[h]
  ...
  \begin {center}
    ...
    \begin{tabular}{l c l l}
      \emph {\textbf {float}} & $\leftarrow$ & \alpha,
        & an IEEE 754 floating point number \\
      & ~ & \alpha \, \in \, \{\textbf {F}\}, \\
      & ~ & \textbf {F} \, = \, \{IEEE 754 floating point numbers\}
    \end{tabular}
    
    \begin{tabular}{lcll}
      \emph {\textbf {string}} & $\leftarrow$ & \texttt {"}\alpha\texttt {"}, 
        & a Unicode string represented as an HJSON string type \\
  ...
  \begin {center}
    ...
    \begin{tabular}{lcll}
      ...
      & ~ & \texttt{comp: [ $\omega_1\, \omega_2\, \omega_3\, \ldots$ ]} 
        & $\omega_i$\, \in\, \{\textbf{O}\}, \\
      & ~ & \} 
	&  \textbf{O} = \{\emph{\textbf{object}}s represented by \textit{objid}s\} \\
    \end{tabular}
    ...
    \begin{tabular}{lcll}
      ...
      & ~ & \texttt{fn: $f$}
        & $f$ \, \in \, \{\textbf{O}\}, \\
      & ~ & \texttt{env: [ $\omega_1\, \omega_2\, \omega_3\, \ldots$ ]} 
        & $\omega_i$\, \in\, \{\textbf{O}\}, \\
      & ~ & \} 
	&  \textbf{O} = \{\emph{\textbf{object}}s represented by \textit{objid}s\} \\
    \end{tabular}
