...
\ref{fig:hjson-object-contents}. The \textit{payload-kind} there is
...
objects have \texttt{payload: class} as a \textsc{Hjson} field in
...
The \textsc{Hjson} representation of payloads vary. The figure
\ref{fig:hjson-class-payload} explains class related payload.

\begin{figure}[h]
  \begin {center}
    \begin{tabular}{llll}
      \emph{\textbf{class-payload}} & : & ~ & \\
      ~ & \texttt{payload:} \texttt{class} &  \\
      ~ & \texttt{superclass:} \textit{object} & the oid of the superclass \\
      ~ & \texttt{methodict:} \textbf{\texttt{[}} &   \emph{\textbf{method-entry}} \ldots  \textbf{\texttt{]}} \\
      \hline \\
    ...
    \end{tabular}
  \end {center}
  \caption {Syntax of class payload}
\label{fig:hjson-class-payload}  
\end{figure}
