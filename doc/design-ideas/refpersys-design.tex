...
\begin{titlepage}
  ...
  \href{http://hop.inria.fr/}{\textsc{Hop.js}},
...
planet-wide issues, such as global warming, demographic and political crises,
economic and financial emergencies, and growing inequalities. In the light of
...
perhaps decades\footnote{An interesting parallel could be controlled
  nuclear fusion -which also bears some ``bootstrapping'' concepts-
...
reverse-engineering that brilliant \textsc{Caia} system.}  by Jacques
...
machinery\footnote{Perhaps inspired by simple designs like
  \href{https://docs.djangoproject.com/en/2.2/topics/templates/}{\textsc{Django}
    templates}, but driven by frame-based {\RefPerSys} objects.}, withe
the ability, like
...
\href{https://en.wikipedia.org/wiki/Symbolic\_artificial\_intelligence}{symbolic
  artificial intelligence} system, it is targeting mostly
...
\begin{figure}[h]
  ...
  \vspace{1em}
    ...
    $\Omega$ ``some code chunk'' $\equiv$ \texttt{\_59RzaVi7hKb00xkX1S}
  \end{center}
  \caption{An example of object representing some code chunk}
...
\textbf{separator} = ``,'' | \textbf{LF} (* line feed *) ; \\
\textbf{oid} = ``\_'' , \{ \textbf{BASE62-CHARACTER} \} ; \\
\textbf{value} = \textbf{int-value} | \textbf{float-value} | \textbf{string-value} 
	       | \textbf{set-value} | \textbf{tuple-value} ; \\
\textbf{int-value} = ``\{'' , ``vtype'' , ``:'' , ``int'' , \textbf{separator} ,
	           ``value'' , ``:'' , \textbf{INTEGER} | \textbf{NIL} ,
		   \textbf{separator} , ``\}'' ; \\
\textbf{float-value} = ``\{'' , ``vtype'' , ``:'' , ``int'' , \textbf{separator} ,
	           ``value'' , ``:'' , \textbf{FLOAT} | \textbf{NIL} ,
		   \textbf{separator} , ``\}'' ; \\
\textbf{string-value} = ``\{'' , ``vtype'' , ``:'' , ``int'' , \textbf{separator} ,
	           ``value'' , ``:'' , \textbf{STRING} | \textbf{NIL} ,
		   \textbf{separator} , ``\}'' ; \\
\textbf{set-value} = ``\{'' , ``vtype'' , ``:'' , ``set'' , \textbf{separator} , 
                   ``value'' , ``:'' , 
		   ``['' , \{ \textbf{oid} (* unique unordered *) , \textbf{separator} \} 
		   | \textbf{NIL} , ``]'' , \textbf{separator} , ``\}'' ; \\
\textbf{tuple-value} = ``\{'' , ``vtype'' , ``:'' , ``tuple'' , \textbf{separator} , 
                   ``value'' , ``:'' , 
		   ``['' , \{ \textbf{oid} (* ordered *) , \textbf{separator} \} 
		   | \textbf{NIL} , ``]'' , \textbf{separator} , ``\}'' ; \\
\textbf{closed-value} = \textbf{int-value} | \textbf{float-value} | \textbf{string-value}
                      | \textbf{set-value} | \textbf{tuple-value} ;
\textbf{closure-value} = ``\{'' , ``vtype'' , ``:'' , ``closure'' , \textbf{separator} ,
		       ``function'' , ``:'' , \textbf{oid} , \textbf{separator} ,
