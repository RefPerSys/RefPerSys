...
\item a class information (inside classes,
  ...
  and the dictionary of methods (see \S
...
\begin{figure}[h]
  \begin {center}
    \begin{tabular}{llll}
      ...
      ~ & \texttt{"class\_methodict":} \textbf{\texttt{[}} &   \emph{\textbf{method-entry}} \ldots  \textbf{\texttt{]}} & method dictionary \\
      \hline \\
...
Given the lack of popularity of Lisp syntax in 2020 or 2021, the
syntax of the REPL language should be closer to JavaScript or to
Python, even if the semantics of \RefPerSys~ is heavily
Lisp-inspired. See markup file
\href{https://github.com/RefPerSys/RefPerSys/blob/master/doc/repl.md}{doc/repl.md}
for details.
...
dictionary (of \textit{oid \texttt{\_627ngdqrVfF020ugC5}})
...
\section{The Web interface of \RefPerSys}


In commits around \texttt{6d44cba00aa9b0a51} (May 2021) a Web
interface is worked upon (but buggy). You might run the
\texttt{./refpersys -AWEB,REPL -W.} (or maybe just \texttt{./refpersys
  -W.}) shell command and browse the
\href{http://localhost:9090}{\texttt{localhost:9090}} URL. The file
\texttt{webroot/index.html.rps} is then served, and hopefully should be template-expanded. Inspired by PHP, processing instructions like
\begin{relsize}{-1.8}
\begin{verbatim}
<?refpersys suffix='rpshtml' action='_2sl5Gjb7swO04EcMqf' rps_json='{"foo":1}'?>
\end{verbatim}
\end{relsize}
should be expanded by closure applications\footnote{In this example,
the applied closure has as connective the object of oid
\texttt{\_2sl5Gjb7swO04EcMqf}. See our C++ function
\texttt{rps\_serve\_onion\_expanded\_stream} in file
\texttt{httpweb\_rps.cc} for details.}. However, such processing
instructions should be on one single line (for example in file
\texttt{webroot/index.html.rps}).

The advantage of a Web interface is at first to be able to generate and use HTML5, and hopefully to generate web forms.

In May 2021 the \texttt{refpersys} program should not be used as a web
server accessible to outside, there are cybersecurity concerns. It
could in a month be run on some isolated laptop to hopefully make a
tiny demo. With websockets, some JavaScript code could be generated
and running in the web browser, in parallel of the agenda mechanism
running inside \texttt{refpersys} process. This has to be implemented,
hopefully with generated C++ code which would generate JavaScript code
and send it to web browser, which would use websockets to communicate
asynchronously with the \texttt{refpersys} process.

At the initial stage, we aim primarily to allow the user to enter C++
code for a specific plugin, and to display the details for a selected
object.

In the case of the former (entering C++ code), we would require to
take advantage of Javascript code editing plugin, such as that
provided by \href{http://codemirror.net}{\texttt{codemirror.net}}. We would need to add the
\texttt{codemirror.js} file to the \texttt{webroot/js/} directory, and
include it from our \texttt{index.html.rps} file. We would then create
a textarea in our index HTML file, and apply the codemirror API on
it. We would also require an HTTP POST endpoint in the RefPerSys web
server that can process the plugin code received by it.

In the case of the latter (displaying RefPerSys objects; there are two
approaches. The first approach is a quick way of displaying existing objects in
the heap store, and is not necessarily the best approach; however, for the
purpose of an initial demo, this approach could work. The second approach is
more involved, but is also probably closer to we would have in mind.

In the first approach, we would serve the index page with a generated JS array
containing the details of all the objects in the object heap. Additionally, the
generated index page will contain a \href{https://select2.org}{\textit{select2}}
widget. The \textit{select2} widget takes care of displaying the full object ID
when the first few characters have been typed in.

\textbf{TODO}


Handling of HTTP requests is done with the help of
\texttt{libonion}\footnote{an HTTP server open source library from
\href{https://coralbits.com/static/onion/}{\texttt{coralbits.com/static/onion/}}}. We
handle only \texttt{GET}, \texttt{POST}, and \texttt{HEAD} methods in
HTTP requests.

For cybersecurity reasons, an URL should not contain \texttt{..} -
this ensures that the file system is not ``overflowed'' by malicious
URLs having many {../..}. As usual, we have a webroot, the Linux
directory containing plain files (like JSON files for Jquery, images,
...).
