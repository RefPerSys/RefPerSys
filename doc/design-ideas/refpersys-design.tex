...
arguments.}, which can be either ``light'' immutable values or
  ...
  For pragmatical reasons, our values should be ordered and hashed,
  since many data structures \cite{cormen:2009:introduction} either
  uses some ordering
  (e.g. \href{https://en.wikipedia.org/wiki/Red-black_tree}{red-black
...
Since objects are fundamental, we want to keep agglomerations of
them. In particular, {\RefPerSys} will reify tuples of objects and
sets of objects as values, and they are the common composite values of
\RefPerSys. A tuple is obviously represented by boxing a sequence of
object references (i.e. pointers). A set would be represented by an
ordered sequence of objects, with membership efficiently testable by a
\href{https://en.wikipedia.org/wiki/Time\_complexity}{$O(log~ n)$} time
...
in our GC, as a last resort mesure!} etc \ldots. Of course every
object has its class (which is itself an object, having a
