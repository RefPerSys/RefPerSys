...
reify sorted dictionnaries mapping strings to values, and their
...
its behavior by the set of slelector it is understanding for message
...
\begin{itemize}
\item the sequence of its direct then indirect superclasses
\item a flexible
  ...
  (perhaps using caching techniques local to each occurence of
  sending).
\end{itemize}
...
\textsc{Java}. Every message send has a reciever $\rho$ (the target of
...
\begin{itemize}
\item let $\kappa$ be initially the class of $\rho$, the reciever or target.
...
(perhaps using some \textsc{message-not-understood} builtin selector à
