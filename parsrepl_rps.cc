...
#warning code review needed. Do we need  this->gc_mark(*gc) in Rps_TokenSource::parse_expression ?
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression¤" << callnum << "#"  << exprnum << " leftv=" << _f.leftv
                << " disjvect:" << disjvect
                <<  " position:" << position_str()<< " startpos:" << startpos);
  bool again = false;
          ...
          if (okright)
            {
              ...
              RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression¤" << callnum << "#"  << exprnum << " after parsedisjright rightv=" << _f.rightv
                            << " disjvect:" << disjvect
                            << " position:" << position_str()
                            << " startpos:" << startpos
                            << "  in:" << (*this));
            ...
            {
              RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression¤" << callnum << "#"  << exprnum << " failing_C at startpos:" << startpos
                            << "  in:" << (*this)
                            << " position:" << position_str()
                            << " curcptr:" << Rps_QuotedC_String(curcptr())
                            << std::endl
...
#warning code review needed. Do we need  this->gc_mark(*gc) in Rps_TokenSource::parse_disjunction ?
  ...
  if (ok)
    {
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_disjunction¤" << callnum << " after leftconj leftv=" << _f.leftv
      ...
      if (again)
        {
          ...
          if (okright)
          ...
          else
            {
              ...
              RPS_PARSREPL_FAILURE(&_,
                                   "Rps_TokenSource::parse_conjunction¤" << callnum << " failing_C at startpos:" << startpos
                                   << " in:" << (*this)
                                   << " position:" << position_str()
                                   << " curcptr:" << Rps_QuotedC_String(curcptr())
                                   << " token_deq:" << toksrc_token_deq);
...
#warning unimplemented Rps_TokenSource::parse_comparand
  ...
  RPS_PARSREPL_FAILURE(&_,
                       ...
                       << " curcptr:" << Rps_QuotedC_String(curcptr())
                       << " token_deq:" << toksrc_token_deq);
  RPS_FATALOUT("missing code in Rps_TokenSource::parse_comparand¤" << callnum << " from " << Rps_ShowCallFrame(callframe)
...
#warning code review needed. Do we need  this->gc_mark(*gc) in Rps_TokenSource::parse_term ?
  ...
  std::string startpos = position_str();
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_term¤" << callnum << " begin startpos:"
                << startpos << "  in:" << (*this)
                << " token_deq:" << toksrc_token_deq
                << " curcptr:" << Rps_QuotedC_String(curcptr()));
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_term¤" << callnum << " START startpos:" << startpos << " divdelimob:" << _f.divdelimob
  ...
  if (!can_start_primary(&_))
    {
      ...
      if (pokparse)
        *pokparse = false;
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_primary¤" << callnum << " failing_B at startpos:" << startpos
                    << "  in:" << (*this)
                    << " position:" << position_str()
                    << " curcptr:" << Rps_QuotedC_String(curcptr())
                    << std::endl
...
Rps_TokenSource::can_start_primary(Rps_CallFrame*callframe)
{
  ...
  if (_f.lexkindob == RPS_ROOT_OB(_2A2mrPpR3Qf03p6o5b) // int
    ...
    {
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::can_parse_primary¤" << callnum << " delimiter "
      ...
      if (_f.lexvalv.is_object())
        {
          ...
          RPS_DEBUG_LOG(REPL, "Rps_TokenSource::can_parse_primary¤" << callnum << " delimob "
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::can_parse_primary¤" << callnum <<" fail lexkindob=" << _f.lexkindob
                << " lexvalv=" << _f.lexvalv);
  return false;
