 ...
 *
 * You can consider RefPerSys as either GPLv3+ or CeCILL or LGPLv3+
 ...
 *
 * Equivalent license in France: CeCILL
 *    on https://cecill.info/licences.fr.html
 *
...
extern "C" void rps_debug_warn_at_msg(const char*file, int line, const char*msg=nullptr);
...
#define RPS_WARNOUT_AT_BIS(Fil,Lin,...) do {                    \
  ...
  rps_debug_warn_at_msg(Fil, Lin, outl##Lin.str().c_str()); \
    };  } while(0)
...
#define RPS_ASSERTPRINTF(Cond,Fmt,...)  do { if (false && (Cond)) \
      fprintf(stderr, Fmt "\n", ##__VA_ARGS__); } while(0)
#define RPS_ASSERT_CALLFRAME(Callframe) do{}while(0)
...
/// notice: on Linux the XDG_SESSION_TYPE environment variable could
/// be used to detect a display server...  It could be a string like
/// "x11" or "wayland".  When missing (e.g. when running batch or
/// crontab scripts) no graphical display is possible.
...
public:
  ...
  //// TODO: interpret a REPL block; may throw an exception
  void interpret_repl_block(Rps_ObjectRef blockob,Rps_ObjectRef envob);
  ///
...
extern "C" void rps_interpret_repl_block(Rps_CallFrame*callframe, Rps_ObjectRef blockob,Rps_ObjectRef envob);
