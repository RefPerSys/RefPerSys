...
#warning win_objref should be known to the garbage collector
{
  ...
  RPS_DEBUG_LOG(GUI, "RpsQWindow::RpsQWindow settings path: "
                ...
                << " "
                << label_setting_name);
  QString label_fontname = qst->value(label_setting_name).value<QString>();
  RPS_DEBUG_LOG(GUI, "RpsQWindow::RpsQWindow settings label_fontname='"
                << label_fontname.toStdString() << "'");
  QFont label_font = QFont(label_fontname);
...
RpsQWindow::create_winobj(Rps_CallFrame*callerframe)
{
  RPS_LOCALFRAME(nullptr /*no descr*/,
                 callerframe,
                 Rps_ObjectRef obwin;
                 Rps_ObjectRef obwinclass;
                );
  RPS_ASSERT(!win_objref);
  RPS_ASSERT(win_command_textedit);
  RPS_ASSERT(win_output_textedit);
  _.obwinclass = rpskob_1DUx3zfUzIb04lqNVt /*rps_window class*/;
  RPS_ASSERT(_.obwinclass /*rps_window is a class*/);
  RPS_ASSERT(_.obwinclass->get_classinfo_payload() != nullptr);
  _.obwin =  Rps_ObjectRef::make_object(&_, _.obwinclass);
